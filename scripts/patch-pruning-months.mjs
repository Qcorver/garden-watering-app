 ...
 *   DELAY_MS=500    — ms between Perenual API calls (default 400)
 */
...
async function main() {
  // Load all plants with empty pruning_months OR empty sunlight
  const { data: plants, error } = await supabase
    .from("plant_species")
    .select("id, perenual_id, scientific_name, common_name_en, pruning_months, sunlight")
    .order("id");
  ...
  console.log(`Plants with missing pruning_months or sunlight: ${targets.length}\n`);
  ...
  for (let i = 0; i < targets.length; i++) {
    ...
    try {
      ...
      if (e.message.includes("429")) {
        console.log("  Rate limited — waiting 5s before continuing…");
        await sleep(5000);
      }
      continue;
    }
